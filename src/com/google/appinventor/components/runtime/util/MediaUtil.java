...
//            FileUtil, SdkLevel, HoneycombMR1Util
...
_L2:
        ...
        options = getBitmapOptions(form, inputstream, s);
        inputstream.close();
        obj = openMedia(form, s, ((MediaSource) (obj)));
        Log.d("MediaUtil", (new StringBuilder()).append("mediaPath = ").append(s).toString());
        s = new BitmapDrawable(decodeStream(((InputStream) (obj)), null, options));
        s.setTargetDensity(form.getResources().getDisplayMetrics());
        if (options.inSampleSize != 1)
        {
            break MISSING_BLOCK_LABEL_122;
        }
        f = form.deviceDensity();
        if (f != 1.0F)
        {
        ...
        inputstream.close();
        throw form;
        int i = (int)(form.deviceDensity() * (float)s.getIntrinsicWidth());
        int j = (int)(form.deviceDensity() * (float)s.getIntrinsicHeight());
        Log.d("MediaUtil", (new StringBuilder()).append("form.deviceDensity() = ").append(form.deviceDensity()).toString());
        Log.d("MediaUtil", (new StringBuilder()).append("originalBitmapDrawable.getIntrinsicWidth() = ").append(s.getIntrinsicWidth()).toString());
        Log.d("MediaUtil", (new StringBuilder()).append("originalBitmapDrawable.getIntrinsicHeight() = ").append(s.getIntrinsicHeight()).toString());
        s = new BitmapDrawable(Bitmap.createScaledBitmap(s.getBitmap(), i, j, false));
        s.setTargetDensity(form.getResources().getDisplayMetrics());
        System.gc();
        form = s;
...
_L3:
    ...
    private static android.graphics.BitmapFactory.Options getBitmapOptions(Form form, InputStream inputstream, String s)
    {
        ...
        inputstream = ((WindowManager)form.getSystemService("window")).getDefaultDisplay();
        int i;
        int j;
        int k;
        ...
        {
            j = (int)((float)inputstream.getWidth() / form.deviceDensity());
            i = (int)((float)inputstream.getHeight() / form.deviceDensity());
        }
        for (k = 1; l / k > j && i1 / k > i; k *= 2) { }
        form = new android.graphics.BitmapFactory.Options();
        Log.d("MediaUtil", (new StringBuilder()).append("getBitmapOptions: sampleSize = ").append(k).append(" mediaPath = ").append(s).append(" maxWidth = ").append(j).append(" maxHeight = ").append(i).append(" display width = ").append(inputstream.getWidth()).append(" display height = ").append(inputstream.getHeight()).toString());
        form.inSampleSize = k;
        return form;
    }
...
_L7:
        ...
        {
            form = HoneycombMR1Util.openContactPhotoInputStreamHelper(form.getContentResolver(), Uri.parse(s));
        } else
        {
