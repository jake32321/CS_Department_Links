    ...
    implements ActivityResultListener
{
    ...
    protected ContactPicker(ComponentContainer componentcontainer, Uri uri)
    {
        ...
        {
            intentUri = HoneycombMR1Util.getContentUri();
            return;
        }
        if (SdkLevel.getLevel() >= 12 && uri.equals(android.provider.Contacts.Phones.CONTENT_URI))
        {
            intentUri = HoneycombMR1Util.getPhoneContentUri();
            return;
        } else
        {
            intentUri = uri;
    ...
    public void ViewContact(String s)
    {
        if (contactUri != null)
        {
            s = new Intent("android.intent.action.VIEW", Uri.parse(s));
            if (s.resolveActivity(activityContext.getPackageManager()) != null)
            {
                activityContext.startActivity(s);
            }
        }
    }
    ...
    public void postHoneycombGetContactEmailAndPhone(Cursor cursor)
    {
        ...
        {
            int i = HoneycombMR1Util.getPhoneIndex(cursor);
            int j = HoneycombMR1Util.getEmailIndex(cursor);
            int k = HoneycombMR1Util.getMimeIndex(cursor);
            String s = HoneycombMR1Util.getPhoneType();
            String s1 = HoneycombMR1Util.getEmailType();
    ...
    public String postHoneycombGetContactNameAndPicture(Cursor cursor)
    {
        ...
        if (cursor.moveToFirst())
        {
            int i = HoneycombMR1Util.getIdIndex(cursor);
            int j = HoneycombMR1Util.getNameIndex(cursor);
            int k = HoneycombMR1Util.getThumbnailIndex(cursor);
            int l = HoneycombMR1Util.getPhotoIndex(cursor);
...
_L5:
        ...
        obj = cursor;
        obj2 = obj4;
        obj3 = HoneycombMR1Util.getDataCursor(s, activityContext, DATA_PROJECTION);
        intent = cursor;
        obj1 = obj3;
        obj = cursor;
        obj2 = obj3;
...
_L9:
        ...
        obj = cursor;
        obj2 = obj3;
        Log.i("ContactPicker", (new StringBuilder()).append("Contact name = ").append(contactName).append(", email address = ").append(emailAddress).append(",contact Uri = ").append(contactUri).append(", phone number = ").append(phoneNumber).append(", contactPhotoUri = ").append(contactPictureUri).toString());
        if (cursor != null)
        {
            cursor.close();
        }
